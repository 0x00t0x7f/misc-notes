...
# 数据校验
+ 校验不可信源传递的外部数据。如一些敏感的api接口，除了前后端校验参数外，也要做好数据完整性的校验，采用白名单机制，放行部分无需鉴权的接口（登录、注册等），其他一律禁止未经授权的访问。
+ 禁止使用os.system等函数调用系统命令
+ 禁止使用 exec、eval等函数执行不可信的代码，改用安全的库函数
+ 禁止在命令解析器中使用通配符。如 os.system("rm -rf /*") 删库跑路
+ 禁止使用subprocess模块中shell=True选项。可能引发兼容性问题。
+ 建议在调用外部可执行程序时使用绝对路径
+ 禁止直接使用外部数据拼接SQL语句。可能会引发SQL注入等安全风险，除了对外部数据做校验之外，在用orm框架或者sql库执行sql前对参数进行转义。
+ 同理，禁止使用 .format()进行格式化外部数据。
+ 禁止使用外部数据拼接XML，在处理解析XML数据时注意不可信的实体被解析而引发安全隐患
+ 防止正则表达式可能引起的redos攻击

# 日志
+ 禁止直接使用外部数据记录日志
+ 使用 loggin模块记录日志建议使用 懒插值方式记录日志，避免预先拼接日志字符串或%格式化
+ 敏感数据在记录日志前需脱敏。如口令、密钥、电话、身份证号等信息。

# 性能&资源管理
+ 合并大字符串时注意性能，建议选择合适的合并方式
+ 建议使用生成器代替一次性生成数据的容器，尤其是在内存敏感场景下的大文件处理
+ 在预知某列表容量的情况和下，在创建该列表时建议提前申明预留足够的空间。如 已知某个列表最多有500个元素，为了避免在添加元素时的频繁扩容开销，提高程序效率，可以直接初始化500个cell，L = [None] * 500

# 测试
+ 禁止用断言 assert验证参数或语句的正确性，在发布至生产环境下应将其删除
+ 在使用测试框架时，测试用例类应使用对应的方法做值判断

# 安全
+ 敏感数据在信任边界外的传输需要签名并加密
+ 建议使用密码学意义上的安全随机数
