...
+ 表现层（接口层）：负责与其他服务，系统交互。处理传入数据的定义、校验、编解码、序列化操作。入参为DTO来协助数据转换，同时与领域对象、实体等解耦。如RESTful接口、MQS消费方
...
+ 基础设施层：提供技术支持，实现仓储接口，封装第三方服务（如缓存、消息队列）、工具类、网关、缓存、数据库存储、消息中间件、监控、应用程序服务等通用的技术和基础服务以及其他服务的防腐。基础设施层以不同方式支持到其他三层，促进各层间通信。
...
+ 领域/子域（Domain/Subdomain）：业务相关知识的集合，业务系统的整体范围，对应一个完整的业务领域。如电商领域、金融领域、短视频领域、社交领域；将领域拆分成多个子域，每个子域对应业务中的一个核心能力；子域又分为三类子域。
  - 核心域：核心业务，需重点建模，决定系统价值。如 电商领域的 订单子域、支付子域就属于核心子域，它直接影响到交易流程
  - 支撑域：辅助业务，为核心子域提供支持，不具备核心竞争力；支撑域具有企业特性，但不具通用性，它是为了实现核心业务而不得不开发的业务所对应的相关知识的集合。可复用现有框架或第三方服务。
  - 通用域：多个子域共用的能力，可抽离为独立组件。可在所有子域中复用；业界已经有成熟方案的业务。同时被多个子域使用的通用功能子域或与行业无关的通用服务
+ 限界上下文：子域的边界，定义子域内的领域模型、统一语言和业务规则，是“领域模型的最小闭环单元”。 跨上下文交互时（如订单上下文调用支付上下文），需通过明确的接口（如支付服务API），而非直接访问对方模型。
+ 上下文映射：描述各限界上下文之间的映射关系（如防腐层、共享内核、开放主机等等）

#### 战术设计（代码层落地）
是指对特定上下文下的模型进行详细设计
战术设计的对象包括聚合、实体和值对象。产出可以是用 UML表达的类图，需要细化到具体的属性，同时确保在代码级别可实现。
在每个限界上下文内，领域模型由以下核心组件构成。
+ 实体： 在相同限界上下文中具有唯一标识（ID）的业务对象或领域模型、生命周期内标识不变，属性可变（如订单、用户），通过标识判断同一性。实体的核心是“状态与行为的封装”；例如 订单实体中应包含“取消订单”的方法（判断当前状态是否可取消，若可取消则更新状态并触发退款事件），而非将“取消逻辑”放在订单服务的接口中。
+ 值对象：无唯一标识，仅通过属性值判断相等性，且状态不可变的对象。属性决定其身份（如金额、数量、地址等），值对象不可变不可修改、可复用、值对象的作用是简化实体设计，可作为实体属性（但不建议这样做，如将收货地址属性放在订单实体中会导致订单实体属性冗余，而将收货地址用值对象封装后，订单实体只需关联一个“收货地址”值对象）
...
+ 领域服务：封装跨实体/值对象的业务逻辑，也就是业务逻辑不属于某个实体/值对象时（无法归属到单个实体或值对象）可抽象为领域服务（如定价策略），通过领域服务协调多个实体或者值对象来完成复杂业务。通常一个聚合有一个对应的领域服务，组织多个实体实现相对复杂的业务逻辑。
+ 领域事件：记录领域内发生的重要事件，用于实现跨领域/跨上下文的异步通信。事件是系统状态发生的某种客观现象，领域事件是和领域有关的事件。通常由聚合内发起，聚合外监听处理。
+ 仓储：负责实体的持久化（如保存入库，读取数据），隔离领域层与基础设施层。以持久化领域模型为职责的类，抽象持久化操作，负责聚合根的存取、屏蔽底层数据库细节。仓储的目的是屏蔽业务逻辑和持久化基础设施的差异，业务模型和存储解耦。
+ 工厂：负责复杂对象或聚合的创建过程
...
### DDD与C4模型协同
DDD是一种针对复杂业务领域的软件开发方法论，C4模型是一种简单直观的软件架构可视化方法。二者结合可解决DDD落地时的沟通障碍和架构退化问题

#### DDD核心概念与C4层次对应关系表
|DDD概念|C4层次|作用|受众|
|---|---|---|---|
|领域上下文|系统上下文/容器|定义领域边界和上下文映射|产品负责人、架构师|
|聚合|组件|封装领域对象和业务规则|开发团队|
|领域服务|组件|实现跨聚合的领域逻辑|开发团队|
|领域事件|动态图/序列图|展示领域状态变化和通信过程|开发团队，测试工程师|
|上下文映射|系统上下文图|展示上下文之间的依赖关系|架构师、项目管理者|

### 常见问题与最佳实践
#### 常见问题解决
1. 上下文边界划分不清
   + 解决方案：通过事件风暴识别限界上下文
   + 可视化验证：绘制上下文映射图，检查依赖关系
2. 聚合设计过大
   + 解决方案：遵循 一个聚合一个职责 原则
   + 可视化验证：检查组件图中聚合的大小和依赖
3. 领域逻辑泄露到应用层
   + 解决方案：严格分层，通过组件图检查依赖方向
   + 可视化验证：确保领域层不依赖应用层和接口层

#### 最佳实践总结
|实践|描述|可视化方法|
|---|---|---|
|分层架构|严格区分领域层，应用层和接口层|组件图中使用边界划分|
|依赖方向|内层不依赖外层，遵循依赖倒置原则|组件图中检查箭头方向|
|聚合设计|控制聚合大小，确保事务一致性|组件图中聚合边界清晰|
|上下文映射|明确上下文的交互方式|系统上下文图中使用不同样式|
|事件驱动|通过事件实现上下文间通信|动态图中展示事件流|
...
**补充**  
+ Microsoft Visio：Visio 是一个专业的流程图和架构图绘制工具，支持多种图示风格和模板。
+ Lucidchart：这是一个在线的流程图和架构图绘制工具，界面友好，支持多种图示风格和协作功能。
+ Draw.io（也称为Diagram.net）：这是一个免费的在线绘图工具，支持绘制各种类型的图表，包括流程图和架构图。
+ Adobe Illustrator：虽然主要是矢量图形设计工具，但也可以用来绘制复杂的图表和架构图。
+ Google Drawings：这是一个简单的在线绘图工具，适合绘制基本的流程图和架构图。
+ OmniGraffle：这是一个功能强大的绘图工具，适用于Mac和iOS平台，支持绘制各种类型的图表
+ Excalidraw：专为手绘风格设计，支持类似“涂鸦”风格的线条、形状和文字，有在线版本和桌面版，适合快速绘制手绘风格的架构图；与 Draw.io 类似，但更强调“手绘感”，图中这种风格也可能是 Excalidraw 生成的
+ Figma / Adobe XD（配合手绘插件）：也可以实现类似效果，但相对更复杂，通常用于设计稿，不如前两者常见
