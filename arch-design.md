...
+ C4模型将系统从上到下 分为 系统上下文、容器、组件、代码 4层视图；每一层是对上层的展开和完善。层层递进对系统进行描述。
...
#### 代码-程序员视角
放大组件视图，会得到组件的代码视图；该层视图一般使用 UML类图、ER图等。是一个可选的详细级别（通常可以通过IDE等工具按需生成）。 除了最重要最核心最复杂的组件外，不建议将这种详细程度用于其他内容。
...
**【参考链接】**  
+ https://www.cnblogs.com/wzj4858/p/17934274.html
